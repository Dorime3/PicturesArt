...
var forms = function forms() {
  var form = document.querySelectorAll('form'); // получаем формы, инпуты и инпуты для ввода телефонов

  var inputs = document.querySelectorAll('input');
  var upload = document.querySelectorAll('[name="upload"]'); // checkNumInputs('input[name="user_phone"]'); // ввод цифр в поле ввода для телефона

  var message = {
    // сообщения для вывода результата запроса
    loading: 'Загрузка',
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      var statusMessage = document.createElement('div'); // создаем блок, стилизуем, помещаем в конец
      ...
      var formData = new FormData(item); // Собираем наши данные с инпутов. С помощью этого метода формируем их в такую структуру, понятную для сервера. Присваиваем в переменную.

      var api;
      item.closest('.popup-design') || item.classList.contains('calc_form') ? api = path.designer : api = path.question;
      console.log(api);
      Object(_services_requests__WEBPACK_IMPORTED_MODULE_6__["postData"])(api, formData) // наша функция является своего рода промисом, которая может завершиться с разным результатом
      .then(function (res) {
        console.log(res);
...
/* harmony default export */ __webpack_exports__["default"] = (forms); // экспортируем. Затем в main.js импортируем.
      ...
      switch (_context.prev = _context.next) {
        case 0:
          ...
          return regeneratorRuntime.awrap(fetch(url, {
            // отправляем. присваиваем ответ в переменную (!!! await для ассинхронного выполнения кода. Поскольку жс не будет ждать ответа с сервера и следовательно не присовит нашей переменной значение. (будет андефайнд))
            method: "POST",
            // гет или пост. у нас форма - сл-но пост
            body: data
          }));
    ...
    while (1) {
      switch (_context2.prev = _context2.next) {
        ...
        case 2:
          ...
          throw new Error("Couldnot fetch ".concat(url, ", status: ").concat(res.status));
